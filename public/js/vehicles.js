if (window.location.pathname.includes('/panel-control/vehicles')) {
    ...
    function displayVehicles(data) {
        ...
        if (data.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="7" class="text-center">Data not available</td></tr>`;
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            row.innerHTML = `
                ...
                <td>${item.brand}</td>
                <td>${item.color}</td>
                <td>${parseInt(item.is_stolen) === 1 || item.is_stolen === true ? 'Yes' : 'No'}</td>
                <td>
                    <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal"
                        data-bs-target="#editVehicleModal" onclick="showEditVehicleModal(${item.id}, ${index})">Edit</button>
                    <button type="button" class="btn btn-danger btn-sm" onclick="confirmDeleteVehicle(${item.id})">Delete</button>
                </td>
            `;
